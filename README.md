...
* The landscape is separated from the model which acts upon it. Corresponding rationalizations are made to project file to group settings 
  more naturally and increase naming clarity and consistency. Input parsing is also more robust so, if you get something wrong, the odds of
  getting an error rather than input being silently ignored or runs proceeding with obviously wrong values are lower.
* Input parsing is separated from data objects, simplifying support for multiple input formats. Feather (Apache Arrow) is supported, within 
  limitations of Arrow's C# support, as a binary alternative to SQL databases and parsing CSV of other delimited files. Feather simplifies 
  integration with R for iLand project setup and data analysis.
* Output objects and calculations are separated from model calculations and objects, disentangling the object model and reducing unnecessary 
  calculations.
* Timesteps are faster due to more consistent use of synchronous thread-level parallelism (~30% reduction in runtime, depending on settings and
...
### Requirements, Suggestions, Limitations, and Dependencies
...
* GlobalSettings::instance()->settings() and DOM XML reparsing are replaced with conventional XML deserialization which checks for schema
...
* Debugging traces are deprecated in favor of debug assertions, exceptions, and expansion of unit test coverage. As practical, interactions are 
  refactored for increased encapsulation, dead code is removed, and duplicate code consolidated. Renaming for clarity and C# style conformance is 
  substantial.
...
### Developer Notes
Primary development is done with [Visual Studio 2022 Community](https://visualstudio.microsoft.com/) but any other .NET 9.0 toolchain is expected
