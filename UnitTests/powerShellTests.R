...
ggplot(elliottResourceUnitTrajectories) +
  ...
  labs(x = "resource unit TPH", y = "resource unit QMD, cm", color = bquote("live stem\nvolume, m"^3*" ha"^-1)) +
...
# LAI
ggplot(elliottResourceUnitTrajectories) +
  geom_path(aes(x = year, y = lai, group = resourceUnit), alpha = 0.5) +
...
# NPP
ggplot(elliottResourceUnitTrajectories %>% filter(year > 2021)) + # no NPP estimates in year before timestepping starts
...
# biomass components
ggplot(elliottResourceUnitTrajectories) + # in rough order from largest to smallest pools
