...
namespace iLand.World
{
    internal class SoilWaterRetentionMualemVanGenuchten : SoilWaterRetention
    {
        ...
        {
            return (Single.IsNaN(environment.SoilVanGenuchtenAlpha) == false) && (Single.IsNaN(environment.SoilVanGenuchtenN) == false) && (Single.IsNaN(environment.SoilThetaR) == false) && (Single.IsNaN(environment.SoilThetaS) == false);
        }

        public override float GetSoilWaterPotentialFromWater(float soilWaterInMM)
        {
            float soilWaterContent = soilWaterInMM / this.soilDepthInMM;
