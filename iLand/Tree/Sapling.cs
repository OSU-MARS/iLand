...
namespace iLand.Tree
{
    ...
    {
        private ResourceUnitSpecies mResourceUnitSpecies;
        ...
        public void SetRU(ResourceUnitSpecies masterRUS) { mResourceUnitSpecies = masterRUS; }
        ...
        {
            Point  offset = mResourceUnitSpecies.RU.TopLeftLightOffset;
        ...
        {
            int index = (pos_index.X - mResourceUnitSpecies.RU.TopLeftLightOffset.X) * Constant.LightPerRUsize + (pos_index.Y - mResourceUnitSpecies.RU.TopLeftLightOffset.Y);
        ...
        private bool GrowSapling(SaplingTreeOld sapling, Model model, float f_env_yr, Species species)
        {
            Point lightCellPosition = model.LightGrid.IndexOf(sapling.LightPixelIndex);
            ...
            if (delta_h_factor < species.SaplingGrowthParameters.StressThreshold)
            {
                ...
                {
                    // sapling dies...
                    ClearSapling(sapling, model.LightGrid, false); // false: put carbon to the soil
                    return false;
                }
            }
            else
            ...
            // grow
            sapling.Height += (float)(delta_h_pot * delta_h_factor);
            ...
            if (sapling.Height > 4.0F)
            {
                ...
                for (int saplingIndex = 0; saplingIndex < saplingsToEstablish; ++saplingIndex)
                {
                    ...
                    treesOfSpecies.Dbh[treeIndex] = dbh * (float)model.RandomGenerator.Random(1.0 - model.ModelSettings.RecruitmentVariation, 1.0 + model.ModelSettings.RecruitmentVariation);
                    treesOfSpecies.SetHeight(treeIndex, sapling.Height * (float)model.RandomGenerator.Random(1.0 - model.ModelSettings.RecruitmentVariation, 1.0 + model.ModelSettings.RecruitmentVariation));
                ...
                ClearSapling(sapling, model.LightGrid, true); // remove this tree (but do not move biomass to soil)
        ...
        public void CalculateGrowth(Model model)
        {
            ...
            mResourceUnitSpecies.Calculate(model, true); // calculate the 3pg module (this is done only if that did not happen up to now); true: call comes from regeneration
