        ...
        protected override void LogYear(Model model, SqliteCommand insertRow)
        {
            foreach (ResourceUnit resourceUnit in model.Landscape.ResourceUnits)
            {
                foreach (ResourceUnitTreeSpecies ruSpecies in resourceUnit.Trees.SpeciesAvailableOnResourceUnit)
                {
                    ...
                    insertRow.Parameters[3].Value = ruSnagStatisticsForSpecies.TreesPerHa;
                    insertRow.Parameters[4].Value = ruSnagStatisticsForSpecies.AverageDbhInCm;
                    insertRow.Parameters[5].Value = ruSnagStatisticsForSpecies.AverageHeightInM;
                    insertRow.Parameters[6].Value = ruSnagStatisticsForSpecies.StemVolumeInM3PerHa;
                    insertRow.Parameters[7].Value = ruSnagStatisticsForSpecies.BasalAreaInM2PerHa;
                    insertRow.Parameters[8].Value = ruSnagStatisticsForSpecies.TreeNppPerHa;
                    insertRow.Parameters[9].Value = ruSnagStatisticsForSpecies.TreeNppPerHaAboveground;
